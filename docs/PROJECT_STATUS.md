...
Full suite: **901 collected** — soul layer **685**, adapter layer **199**,
...
the suite stood at 726, so the figure today is **898 + 3**. Stated that way because
...
dmn 44,
**graph_manager 81** (+6 item 36a's Purpose halves, +4 item 36c's ceiling, +10 item 36e's recurrence selection),
llm_interface 41,
...
state_manager 38,
visual_layer 24 — **685**. Plus **`pad_restore_boundary` 17** (+6, item 30),
...
08-24 for ResLog 26's routing values). Total 901.
...
**Six rows, re-counted from the table above** (`needs-ruling` 4,
...
| Claim | Verified |
|---|---|
| Full suite **901 collected**; soul layer **685**, adapters 199, cross-cutting **17** (was 812 / 602 / 11 — +9 ResLog 29, +6 item 30, +11 item 31, +4 item 32, +21 item 34, +38 across item 36a–e; every one of those is SOUL-layer, so cross-cutting has not moved since item 30) | `make check` (suite + bundle sync); per-file `pytest --collect-only -q` on 2026-08-26. Reads 898 passed + 3 skipped when Ollama is down; COLLECTED is quoted since it does not depend on the machine |
