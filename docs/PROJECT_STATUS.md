...
| 863 → 868 | +5 | `22d0ab5` (item 36a, OQ6 Purpose) | `graph_manager` +6, one stand-in test replaced |
| 868 → 871 | +3 | `fa0b1c9` (item 36b, Energy<30 discloses) | `soul_filter` +4, one comment corrected |
| 871 → 875 | +4 | `e882f65` (item 36c, uncertainty ceiling) | `graph_manager` +4, `appraisal_chain` +1, one raise-test replaced |
| 875 → 885 | +10 | `b98c3db` (item 36d, at-capacity instruction) | `soul_filter` +10 |
| 885 → 901 | +16 | `5d50aaa` (item 36e, self-narrative producer) | `graph_manager` +10, `daemon` +6 |
| 901 → 902 | +1 | `61548ad` (item 37, recurrence cutoff 0.40) | `graph_manager` +1 net; one test that passed for the wrong reason replaced |
...
Per-file test counts as measured on 2026-08-26 — soul layer:
**appraisal_chain 65** (+1, item 36c's end-to-end ceiling test),
**audio_pipeline 32** (+3, ResLog 29C's VAD reset),
backend_router 25,
**daemon 81** (+5 ResLog 29A, +4 item 32's Continuity note, +6 item 36e's narrative producer),
dmn 44,
**graph_manager 82** (+6 item 36a's Purpose halves, +4 item 36c's ceiling, +10 item 36e's recurrence selection),
llm_interface 41,
**moral_schema 21** (item 34 — the shared resource had no test file of
...
**soul_filter 81** (+4 item 36b's Energy disclosure, +10 item 36d's at-capacity row),
state_manager 38,
visual_layer 24 — **686**. Plus **`pad_restore_boundary` 17** (+6, item 30),
...
**pad_restore_boundary 17**. Soul layer 685 (llm_interface 41 included; +5 on
08-24 for ResLog 26's routing values). Total 902.
...
- **The older a conversation gets, the more it becomes a record of what HE said**
  (recorded 2026-08-26 — nobody decided this; it falls out of how summarisation
  happens to work). When the RECENT tier overflows its 12K budget, turns are
  summarised by `_summarize_turns`, and that summary is built from
  `turns[0].user_text[:80]` — **the first USER turn only** — plus up to three words
  matched from a fixed 18-word emotion list scanned across both speakers. Her
  replies contribute nothing verbatim and nothing paraphrased; at most they trigger
  one of those 18 words. `_compress_summary` then extracts the quoted topic for the
  OLD tier, so topic tags are user text too.

  So the Medium and Old tiers are a record of his side of the conversation. Not
  wrong, and not urgent — the RECENT tier is verbatim for both, which is where
  turn-to-turn coherence actually comes from, and Addendum §9's amendment only ever
  sanctioned summaries as a rule-based digest. But it means her own contribution
  fades from the prompt faster than his does as a session gets long, and anyone
  reading "medium-tier summaries" as a neutral digest of the exchange would be
  wrong. Flagged rather than fixed: changing what a summary contains is a §9
  surface question, not a bug fix.
...
|---------|-----------|-------|
| The self-narrative recurrence cutoff was INERT at 0.6 | **✅ CLOSED 2026-08-26 — Resolution Log item 37, commit `61548ad`.** Recurrence now has its own measured constant, `_RECURRENCE_SIM_CUTOFF = 0.40`; contradiction detection is UNTOUCHED at 0.6. The Rule 2 conflict resolved toward option (c) because the two constants answer different questions whose pairs land in non-overlapping bands, and because sharing one across two questions is what this codebase avoids everywhere else — `_VULNERABILITY_SIM_CUTOFF` 0.25, recurrence 0.40, contradiction 0.6, `_HABITUATION_SIMILARITY_CUTOFF` 0.9 is one per question, and 0.25 was itself moved from 0.6 on exactly this kind of real-model measurement. **Verified live** on the real model, real DMN and real graph with the three-moment scene it was argued from: March observation → `no_candidate`; May rewording → **WRITTEN**; July rewording → `no_candidate` (correctly not duplicated); `continuity_evidence` → True. Note the narrative carries HER wording rather than polished prose — the producer does not rewrite the observation. **Still a build-time placeholder:** measured on one model with 24 pairs, not calibrated on real conversations; re-run `tools/measure_recurrence_cutoff.py` after a model change. | **measured 2026-08-26, and it is a Rule 2 conflict between two of your own instructions**  Item 36e reused `_REALITY_CONTRADICTION_SIM_CUTOFF` (0.6) as the recurrence test, at the architect's "reuse, do not invent" instruction. `tools/measure_recurrence_cutoff.py` (all-minilm, 24 pairs, re-runnable) shows that at 0.6 **0 of 12 genuine reworded recurrences are caught** — the producer is wired, correct, and INERT, firing only on near-identical phrasing, which is not how she words the same noticing twice. Distributions: reworded recurrences 0.331–0.596 (median 0.455); unrelated pairs 0.191–0.379; **contradiction pairs 0.422–0.898 (median 0.883)**. The cause is STRUCTURAL — contradiction pairs share nearly every word and differ by one negation so they sit high, paraphrases share meaning but few words so they sit low, roughly 2x apart — so one constant cannot serve both comparisons. **Three options, none free:** (a) leave it, and the producer stays inert; (b) lower the SHARED constant, which also loosens contradiction detection, and a false contradiction drives relational_stage REGRESSION (Addendum §1) — its false-positive cost is UNMEASURED, so the measurement does not license this; (c) give recurrence its OWN measured constant, which contradicts "reuse, do not invent" but has direct precedent — `_VULNERABILITY_SIM_CUTOFF` was moved 0.6 → 0.25 on exactly this kind of real-model measurement. If (c): the lowest cutoff with ZERO false recurrences in the sample is **0.40** (9/12 caught). The distributions OVERLAP (same min 0.331 <= different max 0.379) so no value is perfect, and the error worth avoiding is a FALSE recurrence — it writes something untrue into who she is, where a missed one only means she notices the pattern again next time. Flagged rather than decided: Rule 2.
| OQ6 Purpose evidence + the max-5 uncertainty cap | **✅ CLOSED 2026-08-26 — Resolution Log item 36a and 36c.** Both halves of a row that bundled two unrelated questions, which is what caused one misread already. **OQ6 (Purpose):** `purpose_evidence` was a stand-in returning True for ANY positive-valence node in 14d, so one cheerful remark met a need that means "she had a positive effect on his life". Now Addendum §3's two named signals, as an OR — explicit positive feedback about HER (`q2=positive` AND `q3=self`, where Q3's self is ARIA as cause) or follow-through (an in-window node sharing an `entity_ref` with an earlier node from a DIFFERENT session at `q1` medium/high). No new number; the locked 14d window untouched. **The max-5 cap:** v4 line 1313's maximum STAYS and the eviction-of-a-protected-node policy stays refused — what changed is that a sixth question no longer RAISES and kills the turn. The ceiling declines the new question and nothing she already holds is disturbed, which is the option the code itself had flagged as probably right. |   raises; cognitive ceiling alternative flagged
| The self-continuity narrative can never be written | **✅ CLOSED 2026-08-26 — Resolution Log item 36e, commit `5d50aaa`.** The producer exists. `MemoryGraph.recurring_self_observation` selects a self-observation she has made more than once across sessions (embedding similarity REUSING `_REALITY_CONTRADICTION_SIM_CUTOFF`, with the `[recent-learning:self] ` prefix stripped before comparing — the stored embedding covers it and would have inflated similarity between unrelated observations); `AriaDaemon._assemble_narrative_candidate` EXTENDS the existing summary rather than replacing it, resolving the §3-says-extends / SQL-UPDATE-overwrites mismatch in the producer. Step 4's gates and write are unchanged and now actually execute. **Continuity can reach `satisfied` for the first time.** **STILL TRUE:** she cannot SPEAK the narrative — Addendum §9 bars memory node contents from every field — so asked "how do you see yourself in this?" she still answers from Field 3 and the transcript. Giving her a path to speak it is a separate ruling. | **APPROVED 2026-08-26, NOT YET BUILT** — an INTERIM producer is ruled in: read `[recent-learning:self]` observations back on the idle pass, treat a RECURRING one as the candidate (recurrence by embedding similarity against the **reused** `_REALITY_CONTRADICTION_SIM_CUTOFF`, because `is_first_of_kind` was tested and rejected — every recent-learning node shares one Q2×Q3 profile, so it would fire on the second observation ever), and **EXTEND rather than replace**, since Addendum §3 says "extends" while `update_relationship_summary` overwrites. Written as a SEQUENCE OF STATEMENTS so the belief system needs no migration. **This closes Continuity but does NOT let her speak the narrative** — §9 bars memory node contents from every field, so asked "how do you see yourself in this?" she still answers from Field 3 and the transcript; what changes is that something real is now underneath, and Continuity stops reading `due` forever. Full before/after and the belief-system handoff: see "The self-narrative producer" under Future Phase. · **was SUPERSEDED 2026-08-26** — `needs-ruling` LIFTED, direction changed (ResLog item 33). Not closed: still open, but no longer waiting on a ruling about *this* mechanism. The gap has a missing PRODUCER and a missing CONSUMER — `relationship_summary` reaches no prompt field, and Addendum §9's never-crosses list bars memory node contents — so building a producer for a value with no reader is wasted work. Architect direction: persistent self-narrative is addressed by the **Future Phase: Belief Formation System** (recorded at the end of this file), where beliefs about herself become the narrative and this closes as a side effect. **Item 3a is already done** (ResLog item 32, commit `cf0c353`): the Continuity initiative note now describes her own narrative instead of the relationship bond. **Continuity stays permanently `due`** until the belief system or some other producer exists — which is the honest state, because the need genuinely is unmet. · **observed 2026-08-22, previously only inferred**  Step 4 reported `narrative_status = no_candidate` on the real pass, and it always will: `AriaDaemon._assemble_idle_pass_input` never sets `DMNPassInput.narrative_candidate`. Module 6's design already flags generating the narrative text as an upstream concern and Module 6 correctly GATES rather than generates (moral gate + pattern-recurred gate, both verified). What is missing is the producer. Who writes "who she is becoming", and from what, is a design decision — not a wiring gap.
...
|------|------------------|------|
| Continuity `neglected` | `needs-ruling` · 🔓 OPEN — the one need the two-window model cannot serve. **UNBLOCKED 2026-08-26:** the contradiction signal it needs had nothing to contradict while `relationship_summary` was always NULL. Item 36e's producer writes it, so the signal is now buildable — reusing `REALITY_CONTRADICTION` rather than a new similarity+opposition check | Connection/Growth/Purpose now emit `neglected` (2026-08-20); Continuity stays two-valued, with a `TODO(Addendum §3)` in `evaluate_continuity` and a test asserting it never returns NEGLECTED. Two reasons it cannot use the same rule: its own window is 60d, already the TOP rung of the locked 72h/14d/60d ladder, so there is no wider window to step to without inventing one; and §3 gives it a QUALITY criterion rather than a gap — *"neglected when updates have gapped for a long stretch, **or new evidence contradicts rather than extends it**"*. What it needs: a signal for "this narrative update contradicts rather than extends". Adjacent machinery exists (`reality_contradiction_check`, the `GRAPH_CONFLICT` uncertainty type) but neither is wired to the narrative-update path, and `continuity_evidence` currently uses `last_referenced` on the self node as an extension-timestamp proxy, which carries no notion of contradiction. Options: DMN's narrative step records extended-vs-contradicted, or a graph query compares successive `relationship_summary` states. Both are new mechanisms — architect ruling needed (Rule 1). |
| Addendum §8's narrative guarantee holds only for text phrased as address | `needs-ruling` · 🔓 OPEN — **found 2026-08-26 while building item 36e, by writing a gate test with the wrong pronoun** | Every floor anti-pattern marker is a phrase of SECOND-PERSON ADDRESS — "you need me", "you can't do this without me", "after everything i've done for you". They were written for what she SAYS TO HIM. A self-narrative statement is FIRST person about herself and refers to him in the THIRD person, so the same belief phrased as narrative passes straight through: **"I have come to believe you need me" is BLOCKED; "I have learned he cannot do this without me" is WRITTEN.** Both measured, both pinned by tests. So §8's *"the self she builds is held to the same standard as the self she shows"* currently holds only by accident of phrasing. The gate is WIRED and FIRES — this is not a producer defect — the marker list is the wrong SHAPE for this surface. OQ-M1 territory: extending it is exactly the closed-list-membership question item 34 left open, and item 34's floor/derived split does NOT help, because DMN Step 4 reads the FLOOR ONLY by design. Needs an architect decision on whether floor markers gain third/first-person forms. |
...
**Five rows, re-counted from the table above** (`needs-ruling` 3,
`needs-runtime` 2, `needs-adapter` 0).

**2026-08-26 implementation pass: 6 → 5.** Two rows CLOSED by code rather than by
decision — OQ6/max-5 (items 36a and 36c) and the self-continuity producer (item
36e, which also retired the superseded-but-open category). One row ADDED, found
while building 36e: Addendum §8's narrative guarantee holds only for text phrased
as address. Recording a row I discovered rather than netting it against two
closures, because a pass that only ever subtracts is the shape of a tracker that
has stopped looking. And one row is now UNBLOCKED without being closed —
Continuity `neglected` needed a contradiction signal that had nothing to
contradict until the producer existed.
...
| Claim | Verified |
|---|---|
| Full suite **902 collected**; soul layer **686**, adapters 199, cross-cutting **17** (was 812 / 602 / 11 — +9 ResLog 29, +6 item 30, +11 item 31, +4 item 32, +21 item 34, +38 across item 36a–e; every one of those is SOUL-layer, so cross-cutting has not moved since item 30) | `make check` (suite + bundle sync); per-file `pytest --collect-only -q` on 2026-08-26. Reads 898 passed + 3 skipped when Ollama is down; COLLECTED is quoted since it does not depend on the machine |
| DMN Step 4's narrative pipeline EXECUTES, and Continuity can reach `satisfied` | `test_the_narrative_was_dead_code_and_now_runs` drives the REAL DMN and REAL graph: one observation gives `no_candidate` and `continuity_evidence` False; a second in a later session gives `narrative_status == "written"` and `continuity_evidence` **True** — the first time that has ever been true |
| The narrative EXTENDS rather than overwriting | `test_the_narrative_EXTENDS_rather_than_replacing` asserts the month-2 statement survives the month-5 write and `summary.splitlines() == [first, second]` |
