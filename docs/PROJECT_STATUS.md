...
Full suite: **758 passed** with an embedding backend reachable. Without one, the
...
**transport_ollama 24** (+1 on 08-24, the Qwen family rung),
**transport_cloud 40**, **audio_adapters 67** (+5 on 08-24 for ResLog 25/27:
the strip, the all-narration case, the probe, dormancy, and the refusal of an
unrecognised direction), **visual_adapters 28** — 181. Cross-cutting:
**pad_restore_boundary 11**. Soul layer 561 + **llm_interface 41** (+5 on 08-24
for ResLog 26's routing values). Total 758.
...
| Claim | Verified |
|---|---|
| Full suite 758 passed; soul layer 561 (547 through the boundary phase, +14 in Module 5 for items 21–22) | `make check` (suite + bundle sync); per-file `pytest --collect-only` |
